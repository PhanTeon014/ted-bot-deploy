...
// Motivos de `connection.update` (close) que significam sessão realmente inválida —
// as credenciais não servem mais, precisa parear de novo. Fora desses, a conexão caiu
// mas as credenciais continuam válidas (inclusive `connectionReplaced`/440: só quer
// dizer que outro socket assumiu no lugar desse, não que a sessão foi invalidada).
// Quem escreve sua própria lógica de reconexão pode usar isso pra decidir quando
// limpar a sessão salva vs. só tentar reconectar — sem essa distinção, é fácil acabar
// apagando um pareamento válido por causa de um 440 e ter que parear tudo de novo.
export const FATAL_DISCONNECT_REASONS = [
...
/** limite máximo de tamanho de mídia/documento por upload — 1GB, igual ao WhatsApp oficial */
