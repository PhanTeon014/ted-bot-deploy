...
function showBanner(botName = "卐 | ᛋᛋ Kenzo ᛋᛋ | 卐 ", tagline = "卐 | ᛋᛋ Kenzo ᛋᛋ | 卐 ") {
...
const status = {
  ...
  info: (text) => {
    status._log('info', '⚡️', colors.info, text);
  },
  
  success: (text) => {
    status._log('success', '✝️', colors.success, text);
  },
  ...
  error: (text) => {
    status._log('error', '⛔️', colors.error, text);
  },
